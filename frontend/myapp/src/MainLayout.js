...
function LayoutWrapper() {
  ...
  const normalizeVoiceCommandText = useCallback((rawText) => {
    ...
    const replacements = [
      ...
      [/\b(vetenery|vitenery|wetenery|vetnery)\b/g, "itinerary"],
      [/\b(generate|create)\s+(iternery|itinary|itenerary|vetenery|vitenery|wetenery)\b/g, "generate itinerary"],
      [/\b(listening|listen)\s+(iternery|itinerary|itinary|vetenery|vitenery|wetenery)\b/g, "generate itinerary"],
      [/\b(add to card|add in cart|add to chart)\b/g, "add to cart"],
      [/\b(remove from card)\b/g, "remove from cart"],
      [/\b(open cap|open card|opening card|listening card|listen card)\b/g, "open cart"],
